(function () {
  ...
  function findDuplicate(isbn) {
    ...
    return (window.state && state.books || []).find(
      book => cleanIsbn(book.isbn) === normalized && book.status !== 'סל מחזור'
    );
  }
  ...
  async function fetchBookByIsbn(rawIsbn) {
    ...
    if (cover && window.importCover) {
      ...
      const storedCover = await window.importCover(cover.replace('http://', 'https://'));
      if (storedCover) {
  ...
  function patchDataMapping() {
    ...
    window.bookToRow = function (book) {
      return Object.assign(originalBookToRow(book), { isbn: cleanIsbn(book.isbn) || null });
    };

    window.saveBook = async function () {
      const field = document.getElementById('isbn');
      const isbn = cleanIsbn(field && field.value);
      if (isbn && !isValidIsbn(isbn)) return window.toast('מספר ה־ISBN אינו תקין');
      ...
      const saved = window.state && state.books.find(book => book.id === document.getElementById('id').value);
  ...
  function initIsbnModule() {
    injectUi();
    patchDataMapping();
    loadScannerLibrary().catch(() => showMessage('רכיב הסריקה לא נטען. הזנה ידנית עדיין זמינה.', 'error'));
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initIsbnModule, { once: true });
  } else {
    initIsbnModule();
  }
})();
