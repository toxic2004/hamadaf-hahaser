...
// Keyset-pagination cursor for scanning through all of a user's active
// books across multiple invocations, not just the same first N every
// time (2026-08-31, closes the gap flagged in the premortem: loadActiveBooks
// previously always did .order("id").limit(N) with no cursor at all, so
// repeated runs only ever re-scanned the same first N books and never
// reached the rest of a 67-75 book list).
//
// book.id is a text UUID (not sequential/chronological), but text UUID
// comparison is still a stable, deterministic total order - exactly
// what keyset pagination needs. It doesn't need to mean anything
// chronologically, only that every book is visited exactly once per
// full cycle before any repeats.
//
// Pulled out as a pure function (no DB access) specifically so this
// logic is unit-testable the normal way, unlike the Deno-specific
// query code around it.
export function nextCursorAfterBatch(booksInBatch, batchLimit) {
  if (!booksInBatch.length) return null; // nothing left - wrap to the start
  if (booksInBatch.length < batchLimit) return null; // reached the end of the list this cycle
  return booksInBatch[booksInBatch.length - 1].id; // more books after this batch
}
